...
<head>
    ...
    <script type="text/javascript">
        ...
        function update(zhe) {
            ...
            // $.post("order?State=Update", {"id": id, "UserName": UserName, "defaults": defaults}, function (Data) {
            //     if (Data == "true") {
                    /* $(zhe) */
        ...
        $(function () {
            ...
            $(".dizhi ul li").click(function () {
                var text = $(this).html();
                $.post("order?State=Select", {"text": text}, function (data) {
                    console.log(data)
        ...
        function city(thiss) {
            var text = $(thiss).html();
            $.post("order?State=SelectQu", {"text": text}, function (data) {
        ...
        $(function () {
            ...
            $("#save").click(function () {
                ...
                $.post("DingDan.do?State=AddAddress", {
                    "Name": Name,
                    "Phone": Phone,
                    "BeiYongPhone": BeiYongPhone,
                    "address1": address1,
                    "address2": address2,
                    "lable": lable,
                    "is": is,
...
<div id="beijings" style="display: none">
    ...
    <div id="context" style="width: 600px;height: 180px;margin-top: 150px;">
        <dl style="width: 520px;line-height: 25px;">
            ...
            <dd style="width: 450px;" id="wen">订单处理成功！您的订单号：${orderInfo.dingdanNumber }，收货人姓名：<span id="mingzi">张三</span><span
                    id="infos"></span></dd>
            <dd style="margin-top: 20px;margin-left: 230px;"><input id="ok" type="button" value="确定"
                                                                    style="padding: 3px 10px;color: #fff;border: 0px;background: #ff3366">
            </dd>
        </dl>
    </div>
</div>
        ...
        <form action="javascript:void(0)" method="post">
            <table border="0px">
                ...
                <tr>
                    ...
                    <td><input type="text" disabled="disabled" name="address2" style="width: 400px;"
                               placeholder="街道、小区、楼牌号等，无须重复填写省市区"></td>
                    <td></td>
                </tr>
                <tr>
                    <td>标签：</td>
                    <td>
                        <biaoqian>家</biaoqian>
                        <biaoqian>公司</biaoqian>
                        <biaoqian>学校</biaoqian>
                    </td>
                    <td></td>
                </tr>
                <tr>
...
<form action="order/pay" method="post" id="tijiaos">


    <input type="hidden" name="dingdannumber" value="${orderInfo.dingdanNumber }">
            ...
            <c:forEach var="address" items="${ShouHuolist }">
                <c:if test="${address.defaults==0 }">
                    <div class="addressinfo" onclick="update(this)" name="addre">
                        ...
                        <div class="wenzi">
                            <p>${address.name }（${address.city }）
                                <lable>${address.lable }</lable>
                                <default>默认</default>
                            </p>
                            <span>
                                    ${address.address }
                            ...
                            <a href="#">修改</a>
                            <a href="javascript:void(0)" onclick="shanchu(${address.id },${address.userName })"
                               class="shanchu">删除</a>
                        </div>
            ...
            <table id="shoptable" border="0px" cellpadding="0px" cellspacing="0px">
                ...
                <c:forEach var="item" items="${list }">
                    ...
                    <td style="border-right: 1px #eee solid;border-top: 1px #eee solid;">
                        <div class="xinxi">
                            <img alt="${item.si.image}" src="${item.si.image}" width="60px" height="60">
                            <span class="biaoti">${item.si.shopName}</span>
                            <p style="color: #999">尺寸：${item.chiCun} 颜色：${item.color}
                            </p>
                            <p style="color: #999">七天无理由退货</p>
                        </div>
                    </td>
                    <td align="center" style="border-right:1px #eee solid;">完成后尽快为您发货</td>
                    <td align="center">￥${item.si.shopPrice}
                    </td>
                    <td align="center">${item.count}
                    </td>
                    <td align="center">￥${item.si.shopPrice*item.count}
                    </td>
                    </tbody>
                    <input type="hidden" name="shopid" value="${item.shopId}">
                    <input type="hidden" name="shopcount" value="${item.count}">

                </c:forEach>
                <input type="hidden" name="maijiauser" value="${orderInfo.userName}">
                <tfoot>
                <tr>
                    <td colspan="2" style="border-top: 1px #eee solid;border-right: 1px #eee solid;">
                        <input type="text" size="110" name="beizhu" onkeyup="state(this)"
                               style="border: 1px #ccc solid;outline: none;font-size: 12px;height: 20px; margin-left: 20px;">
                        <span>0/100</span>
                    </td>
                    <td align="center">总计</td>
                    <td></td>
                    <td align="center">￥${orderInfo.price}
                    </td>
                </tr>
                </tfoot>
            </table>
    ...
    <footer>
        <div id="tijiao">
            <p class="jine">应付金额：<span id="price">${orderInfo.price}.00</span></p>
            <p id="tongyao"><span id="lijian">立减</span>童瑶支付立减5元 <a href="" style="text-decoration: none;color: #28e">点此领取</a>
            </p>
            <p class="jine">收货信息：
                <info style="color:#000;" id="shouhuos">张晨 152****5008 河北省邯郸市永年区广府镇北堤口</info>
            </p>
            <p class="jine">支付方式：
                <info style="color:#000;" id="fukuanfangshi">你还未选择付款方式</info>
            </p>
            <p><span id="ts"><img alt="" src="Image/LeGou_Image/!.png" width="16px" style="vertical-align: middle;"> 提交订单后尽快支付，商品才不会被人抢走哦！</span><input
                    type="button" id="submits" class="tjs" value="提交订单"></p>
        </div>
    </footer>
    <input type="hidden" name="dingdanjine" value="${orderInfo.price}">
</form>
</body>
<script type="text/javascript">
    $(function () {
